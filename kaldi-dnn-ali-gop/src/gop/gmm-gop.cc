                     ...
                     BaseFloatMatrixWriter *phn_frame_conf_writer)
{
  ...
  for (MatrixIndexT i = 0; i < split.size(); i++)
  {
    ...
    BaseFloat gop_numerator = use_viterbi_numera ? ComputeGopNumeraViterbi(split_decodable, phone_l, phone, phone_r) : ComputeGopNumera(ali_decodable, alignment_, frame_start_idx, split[i].size());
