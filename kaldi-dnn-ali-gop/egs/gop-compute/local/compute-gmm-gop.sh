...
if [ -f $dir/phoneme_conf.1 ]; then
  ...
  python2 local/phn_conf_ops.py --phn_conf_file $dir/phoneme_conf.txt --all_phone_file $lang/phones.txt --output_dir $dir/phoneme_conf
fi
if [ -f $dir/phoneme_frame_conf.1 ]; then
  for n in $(seq $nj); do
    cat $dir/phoneme_frame_conf.$n || exit 1;
  done > $dir/phoneme_frame_conf.txt || exit 1
  python2 local/phn_conf_ops.py --phn_conf_file $dir/phoneme_frame_conf.txt --all_phone_file $lang/phones.txt --output_dir $dir/phoneme_frame_conf
fi
