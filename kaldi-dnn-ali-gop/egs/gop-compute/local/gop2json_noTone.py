...
def makePhonesDictWithTone(phonesTxtWithTone):
    with open(phonesTxtWithTone) as f:
        for line in f:
            line = line.strip()
            phone = line.split(' ')[0]
            if phone[0] == '#':
                continue
            i = int(line.split(' ')[1])
            PhonesWithTone[i] = phone
            PhonesWithTone_reverse[phone] = i
...
def wordAlign(text, phones,
    ...
    for word in text:
        ...
        ######### Shanboy 20180127 deal with tone #########

        S = []
        s = []
        for p in rightPhoneSeq:
            if p not in PhonesWithTone_reverse:
                s.append(p)
                S.append(s)
                s = []
            else:
...
if __name__ == '__main__':
    ...
    phonesTxtWithTone = landWithToneDir + '/phones.txt'
    makePhonesDictWithTone(phonesTxtWithTone)
